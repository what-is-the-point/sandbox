#!/usr/bin/env python3
########################################################
#   Title: SBS1 Thread for Razel Dictionary
#  Thread: SBS1 Thread
...
class SBS1_Thread(threading.Thread):
    ...
    def __init__ (self, cfg, parent):
        ...
        self.obs_lat = self.cfg['observer']['lat']
        self.obs_lon = self.cfg['observer']['lon']
        self.obs_alt = self.cfg['observer']['alt']
        self.trk_dict={} 
    ...
    def run(self):
        ...
        while (not self._stop.isSet()):
            ...
            #time.sleep(0.000001)
            time.sleep(0.000001) #<--- I will need to throttle outputs to avoid overwhelming the image processing daemon
    ...
    def _handle_recv_data(self, data):
        try:
            ...
            if msg != None:
                if int(msg['tx_type']) == 3:
                    razel = RAZEL(self.obs_lat, self.obs_lon, self.obs_alt / 1000.0,
                          msg['latitude'], msg['longitude'], msg['altitude']*0.0003048)
                    self.trk_dict[msg['hex_ident']] = [razel['az'],razel['el']]
                    
                    for key in list(self.trk_dict.keys()):
                        if self.trk_dict[key][1] <10:
                            self.trk_dict.pop(key)
                        else:
                            pass
                    """ print('Start dictionry...........')
                    print(self.trk_dict)
                    print('End dictionry...........') """

                    self.tx_q.put(self.trk_dict)
                    #print(msg)
        except Exception as e:

...
class Device_Thread(threading.Thread):
    ...
    def _process_device_command(self, msg):
        ...
        try:
            if "position" in msg['type']:
                ...
                if self.nexstar['cur_el'] > 180: self.nexstar['cur_el']-=360.0
            ...
            elif msg['type'] == 'goto.slew':
                self.nexstar['tar_az']=msg['params']['com_az']
                self.nexstar['tar_el']=msg['params']['com_el']

                self.ns.slew_fixed(nexstar.NexstarDeviceId.AZM_RA_MOTOR,msg['params']['az_rate'])
                self.ns.slew_fixed(nexstar.NexstarDeviceId.ALT_DEC_MOTOR,msg['params']['el_rate'])

            elif msg['type'] == 'stop.slew':
                self.ns.slew_fixed(nexstar.NexstarDeviceId.AZM_RA_MOTOR,0)
                self.ns.slew_fixed(nexstar.NexstarDeviceId.ALT_DEC_MOTOR,0)

            elif 'slew.joystick' in msg['type']:
    ...
    def _connect(self,msg=None):
        ...
        self.tlm['model'] = self.nexstar['model']
        self.tlm['version'] = self.nexstar['version']
        self.tlm['mode'] = self.nexstar['mode']
    ...
    def _init_nexstar(self):
        ...
        self.nexstar['gtip'] = self.ns.getGotoInProgress()
        self.nexstar['cur_az'], self.nexstar['cur_el'] = self.ns.getPosition()
        if self.nexstar['cur_el'] > 95: self.nexstar['cur_el']-=360.0
        vers = self.ns.getVersion()
        self.nexstar['version'] = "{:d}.{:d}".format(vers[0],vers[1])
        self.nexstar['mode'] = self._parse_tracking_mode(self.ns.getTrackingMode())
    ...
    def _parse_tracking_mode(self,mode):
        if mode == 0:
            return "OFF"
        elif mode == 1:
            return "ALT_AZ"
    ...
    def _parse_model(self,model):
        if model == 1:
            ...
            return "NexstarSE 4/5"
        elif model == 12:
            return "NexstarSE 6/8"
        elif model == 15:
            return "lcm"
        else:
            return 'unknown'
